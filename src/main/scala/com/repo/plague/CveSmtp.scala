...
class SessionHandler(connection: ActorRef, socket: ActorRef) extends Actor with FSM[State,EmptyEnvelope]
  ...
  def say(s: String): Unit = socket ! Tcp.Write(ByteString(s + "\n"))
  def whine(): State = {
    stay()
  }
  def cmd(s: String, command: String => Boolean): Boolean = command(s.toUpperCase.trim)
...
class ConnectionHandler(remote: InetSocketAddress,
  ...
  val sessionHandler: ActorRef = context.actorOf(Props(new SessionHandler(connection, socket)))
...
class SmtpService(endpoint: InetSocketAddress) extends Actor with ActorLogging {
  ...
  def receive: PartialFunction[Any, Unit] = {
    case Tcp.Connected(remote, _) =>
      ...
      //TODO: set up connection handler
      val socket = sender
